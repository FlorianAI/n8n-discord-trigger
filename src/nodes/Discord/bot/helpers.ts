...
export const addLog = (message: string, client: Client, nodeId?: string) => {
  const logMessage = nodeId ? `[NodeId: ${nodeId}] ${message}` : message
...
export function registerNodeInstance(nodeId: string) {
  state.registerNode(nodeId)
}

export function unregisterNodeInstance(nodeId: string) {
  state.unregisterNode(nodeId)
}
